...
def findGearsAndIndices(lines: list) -> (list, list):
    gears = []
    indices = []
    ...
    for row_index, row in enumerate(lines):
        for col_index, char in enumerate(row):
            if char == '*':
                gears.append(char)
                indices.append(((row_index, col_index), (row_index, col_index)))
...
def removeNonParts(text: list, parts: list, indices: list) -> list:
    ...
    part_indices = [index for index in indices if isPart(text, index)]
    # print(parts_to_keep, part_indices)
    return [int(part) for part in parts_to_keep], part_indices

def expandPartIndices(indices: list) -> list:
    expanded = []
...
def findGears(text: list, parts: list, partIndices: list) -> list:
    # Skal finde alle stjernene også generateAdjecent Indices
    gears, gearIndices = findGearsAndIndices(text)
    expandPartIndices = expandPartIndices(partIndices)



    for part in parts:
        generateAdjecentIndices()

def calculateSumOfParts(parts: list):
    return sum(parts)

tmp = findNumbersAndIndices(text)
print(calculateSumOfParts(removeNonParts(text, tmp[0], tmp[1])[0]))
